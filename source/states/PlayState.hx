...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		['you suck ass lmao', 0.2], //From 0% to 19%
		['dear god', 0.4], //From 20% to 39%
		['uhm', 0.5], //From 40% to 49%
		['ok i guess?', 0.6], //From 50% to 59%
		['its nice', 0.69], //From 60% to 68%
		['peak', 0.7], //69%
		['improvement', 0.8], //From 70% to 79%
		['acceptable', 0.9], //From 80% to 89%
		['coolio', 0.99], //From 90% to 99%
		['PERFECT', 1] //The value on this one IS used actually, since Perfect is always "1"
	];

	public static var ratingInfoMap:Map<String, RatingInfo> = [
	    "perfect" => { name: "Perfect", color: 0xFF00FFFF }, // cyan
	    "sick"    => { name: "Sick",    color: 0xFF00FF00 }, // green
	    "good"    => { name: "Good",    color: 0xFFFFFF00 }, // yellow
	    "bad"     => { name: "Bad",     color: 0xFFFFA500 }, // orange
	    "shit"    => { name: "Shit",    color: 0xFFFF0000 }  // red
	];
	...
	override public function create()
	{
		...
		waterMark = new FlxText(0, 0, FlxG.width - 800, songName.toUpperCase() + " | Modified PE", 24);
		waterMark.setFormat(Paths.font("vcr.ttf"), 24, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	public dynamic function updateScoreText()
	{
		...
		disScore = Math.ceil(FlxMath.lerp(disScore, songScore, 0.1));

		var tempScore:String;
		if(!instakillOnMiss) tempScore = Language.getPhrase('score_text', 'Score: {1} | Misses: {2} | Rating: {3}', [disScore, songMisses, str]);
	...
	override public function update(elapsed:Float)
	{
		if(!inCutscene && !paused && !freezeCamera) {
			FlxG.camera.followLerp = 0.04 * cameraSpeed * playbackRate * Conductor.bpm / 100;
	...
	{
		var mult:Float = FlxMath.lerp(1, iconP1.scale.x, Math.exp(-elapsed * 9 * playbackRate * Conductor.bpm / 100));
		var dir:Float = FlxMath.lerp(0, iconP1.angle, Math.exp(-elapsed * 9 * playbackRate * Conductor.bpm / 100));
		iconP1.angle = dir;
		iconP1.scale.set(mult, mult);
		iconP1.updateHitbox();

		var mult:Float = FlxMath.lerp(1, iconP2.scale.x, Math.exp(-elapsed * 9 * playbackRate * Conductor.bpm / 100));
		var dir:Float = FlxMath.lerp(0, iconP2.angle, Math.exp(-elapsed * 9 * playbackRate * Conductor.bpm / 100));
	...
	override function sectionHit()
	{
		if (SONG.notes[curSection] != null)
		{
			...
			camZoomingDecay = Conductor.bpm / 100;
